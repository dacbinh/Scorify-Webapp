...
import { GraduationCap, BookOpen, Users, Plus, Settings, LogOut, Edit, Trash2, FileText, Clock, CheckCircle, AlertCircle, Sparkles, Calendar, Upload } from "lucide-react";
...
export function ClassDetailPage() {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<Array<{ name: string; size: number; lastModified: number }>>([]);
  const [pendingFiles, setPendingFiles] = useState<Array<{ name: string; size: number; lastModified: number }> | null>(null);
  ...
  const saveUploads = (files: Array<{ name: string; size: number; lastModified: number }>) => {
  ...
  const confirmUpload = () => {
    ...
    setNotification("Tải lên thành công. Scorify sẽ giữ dữ liệu trong phiên này.");
  ...
  const classes = [
    { id: 1, name: "Toán 10A1", subject: "Toán", students: 42, exams: 6, createdDate: "2025-09-01", description: "Đại số, hình học và luyện tập tư duy giải toán theo chuyên đề" },
    { id: 2, name: "Ngữ văn 10A1", subject: "Ngữ văn", students: 40, exams: 5, createdDate: "2025-09-03", description: "Đọc hiểu văn bản, nghị luận xã hội và nghị luận văn học" },
    { id: 3, name: "Tiếng Anh 10A1", subject: "Tiếng Anh", students: 41, exams: 6, createdDate: "2025-09-05", description: "Phát triển 4 kỹ năng nghe, nói, đọc, viết và ngữ pháp nền tảng" },
    { id: 4, name: "Vật lí 10A2", subject: "Vật lí", students: 38, exams: 5, createdDate: "2025-09-07", description: "Cơ học, nhiệt học và bài tập vận dụng thực tiễn" },
    { id: 5, name: "Hóa học 10A2", subject: "Hóa học", students: 37, exams: 5, createdDate: "2025-09-09", description: "Cấu tạo chất, phản ứng hóa học và bài tập định lượng cơ bản" },
    { id: 6, name: "Sinh học 10A2", subject: "Sinh học", students: 36, exams: 4, createdDate: "2025-09-11", description: "Tế bào học, vi sinh vật và các quá trình sinh học nền tảng" },
    { id: 7, name: "Lịch sử 11A1", subject: "Lịch sử", students: 35, exams: 4, createdDate: "2025-09-13", description: "Lịch sử Việt Nam và thế giới giai đoạn cận đại - hiện đại" },
    { id: 8, name: "Địa lí 11A1", subject: "Địa lí", students: 34, exams: 4, createdDate: "2025-09-15", description: "Địa lí tự nhiên, kinh tế - xã hội Việt Nam và kỹ năng Atlat" },
    { id: 9, name: "Giáo dục công dân 12A1", subject: "GDCD", students: 39, exams: 3, createdDate: "2025-09-17", description: "Pháp luật, quyền và nghĩa vụ công dân, tình huống thực tế" },
    { id: 10, name: "Tin học 12A1", subject: "Tin học", students: 33, exams: 4, createdDate: "2025-09-19", description: "Thuật toán, lập trình cơ bản và ứng dụng công nghệ thông tin" },
  ];
  ...
  const titles = examTitleBySubject[classData.subject] ?? ["Bài kiểm tra chuyên đề"];
  const exams = Array.from({ length: classData.examsCount }, (_, idx) => {
    const status = idx < 2 ? "active" : idx === 2 ? "draft" : "scheduled";
    const submissions = status === "active" ? Math.max(classData.studentsCount - 4 + idx, 0) : 0;
  ...
  const middleNames = ["Minh", "Khánh", "Gia", "Quang", "Thu", "Hoài", "Anh", "Ngọc", "Thanh", "Hải"];
  const lastNames = ["Anh", "Linh", "Nam", "Hân", "Huy", "Mai", "Trang", "Phúc", "Khang", "Vy"];
  ...
  const students = Array.from({ length: classData.studentsCount }, (_, idx) => {
    ...
    return {
      ...
      joined: "2025-09-01",
      submissions: Math.min(exams.filter((e) => e.status !== "scheduled").length, 1 + (idx % Math.max(classData.examsCount, 1))),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <Link to="/classes" className="hover:text-[#F05123]">Lớp học</Link>
            <span>/</span>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600">{classData.subject} • Năm học {classData.academicYear}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-900 font-medium">{classData.studentsCount} học sinh</span>
                </div>
                <div className="flex items-center gap-2">
                  <FileText className="size-5 text-gray-400" />
                  <span className="text-gray-900 font-medium">{classData.examsCount} bài kiểm tra</span>
                </div>
                <div className="flex items-center gap-2">
                  <Calendar className="size-5 text-gray-400" />
                  <span className="text-gray-600">Tạo lúc {new Date(classData.createdDate).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          ...
          <div className="p-8">
            ...
            {activeTab === "exams" && (
              <div className="space-y-4">
                {exams.map((exam) => (
                  <div key={exam.id} className="p-6 rounded-lg border border-gray-200 hover:border-orange-300 hover:shadow-md transition-all">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                      ...
                      <div className="flex gap-2 ml-6">
                        {exam.status === "active" && exam.submissions > exam.graded && (
              ...
              <div>
                <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">Tải lên ảnh bài kiểm tra</h3>
                      <p className="text-sm text-gray-600">
                        Chọn thư mục/tệp chứa ảnh bài kiểm tra của học sinh. Sau khi chọn, bạn sẽ được yêu cầu xác nhận.
                      </p>
                    </div>
                  ...
                  {uploadedFiles.length > 0 && (
                    <div className="mt-6">
                      <div className="text-sm text-gray-600 mb-2">Tệp đã tải lên trong phiên này:</div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {uploadedFiles.map((file, idx) => (
                          <div key={`${file.name}-${file.lastModified}-${idx}`} className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700">
                  ...
                  <table className="w-full">
                    <thead className="border-b border-gray-200">
                      <tr>
                        ...
                        <th className="text-left py-3 px-4 font-medium text-gray-700">Ngày tham gia</th>
                        <th className="text-left py-3 px-4 font-medium text-gray-700">Bài đã nộp</th>
                        <th className="text-left py-3 px-4 font-medium text-gray-700">Điểm TB (/10)</th>
                    ...
                    <tbody>
                      {students.map((student) => (
                        <tr key={student.id} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-4 px-4 text-gray-600">{new Date(student.joined).toLocaleDateString()}</td>
                          <td className="py-4 px-4 text-gray-900">{student.submissions}</td>
                          <td className="py-4 px-4">
            ...
            {activeTab === "submissions" && (
              <div className="space-y-4">
                {recentSubmissions.map((submission) => (
                  <div key={submission.id} className="p-6 rounded-lg border border-gray-200 hover:border-orange-300 hover:shadow-md transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{submission.exam}</p>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`px-4 py-2 rounded-lg text-sm font-medium ${getSubmissionStatusColor(submission.status)}`}>
                          {submission.status === "graded" ? `Điểm: ${submission.grade}/10` : "Chờ chấm"}
                        </span>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4 text-center">Xóa lớp học?</h2>
            <p className="text-gray-600 mb-6 text-center">
              Bạn có chắc muốn xóa "{classData.name}"? Hành động này không thể hoàn tác và sẽ xóa toàn bộ bài kiểm tra cùng bài nộp liên quan.
            </p>
      ...
      {showConfirmUpload && pendingFiles && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-8">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4 text-center">Xác nhận nộp</h2>
            <p className="text-gray-600 mb-6 text-center">
              Bạn có chắc muốn nộp thư mục/tệp này chứ, hãy chắc chắn bạn đã chọn đúng lớp/học sinh để Scorify chấm cho bạn nhé.
            </p>
            <div className="max-h-44 overflow-y-auto mb-6 border border-gray-200 rounded-lg p-4">
              {pendingFiles.slice(0, 10).map((file, index) => (
                <div key={`${file.name}-${file.lastModified}-${index}`} className="text-sm text-gray-700 mb-2">
                  {file.name} • {(file.size / 1024).toFixed(1)} KB
                </div>
              ))}
              {pendingFiles.length > 10 && (
                <div className="text-xs text-gray-500">... và {pendingFiles.length - 10} tệp khác</div>
              )}
            </div>
            <div className="flex gap-4">
              <button
